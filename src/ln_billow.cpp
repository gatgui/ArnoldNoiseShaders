...
shader_evaluate
{
   ...
   case NT_value:
      {
         fBm<ValueNoise, AbsoluteModifier> fbm(octaves, amplitude, persistence, frequency, lacunarity);
   ...
   case NT_perlin:
      {
         fBm<PerlinNoise, AbsoluteModifier> fbm(octaves, amplitude, persistence, frequency, lacunarity);
         fbm.noise_params.seed = AiShaderEvalParamInt(p_seed);
   ...
   default:
      {
         fBm<SimplexNoise, AbsoluteModifier> fbm(octaves, amplitude, persistence, frequency, lacunarity);
         fbm.modifier_params.remap_range = true;
         sg->out.FLT = fbm.eval(P);
      }
   }
